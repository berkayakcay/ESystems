    ...
    class MyClass
    {
        ...
        public class Program
        {
            static void Main(string[] args)
            {
                ...
                // Add list elements  
                MyClass newClass = new MyClass("John Doe", 42);
                myList.Add(newClass);
                newClass = new MyClass("Jane Roe", 36);
                myList.Add(newClass);
            ...
            private static void GetYevmiye()
            {
                        ...
                        AccountingEntries = new AccountingEntries()
                        {
                            DocumentInfo = new DocumentInfo()
                            {
                                ...
                                EntriesComment = new EntriesComment()
                                {
                                    ContextRef = "journal_context",
                                    Text =
                                        "1 Ocak 2017 - 31 Ocak 2017 arası VOLANT YAZILIM Anonim Şirketine ait yevmiye defteri"
                                },
                                        ...
                                        AccountantContactInformation = new AccountantContactInformation()
                                        {
                                            AccountantContactPhone = new AccountantContactPhone()
                                            {
                                                AccountantContactPhoneNumberDescription =
                                                    new AccountantContactPhoneNumberDescription()
                                                    {
                                                        ContextRef = "journal_context",
                                                        Text = "bookkeeper"
                                                    },
                ...
                CreateCSVFromGenericList(list,
                    $"{Directory.GetCurrentDirectory()}\\VOLANT\\eSystems\\eDefter\\export.csv");

                folderExists = false;

                folderExists =
                    Directory.Exists(
                        $"{Directory.GetDirectoryRoot(Directory.GetCurrentDirectory())}VOLANT\\eSystems\\eDefter");
                if (!folderExists)
                {
                ...
                CreateCSVFromGenericList(list,
                    $"{Directory.GetDirectoryRoot(Directory.GetCurrentDirectory())}VOLANT\\eSystems\\eDefter\\export.csv");
